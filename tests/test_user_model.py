...
class UserModleTestCase(unittest.TestCase):
    def setUp(self):
        ...
        db.create_all()
        # insert basic role into roles table
        Role.insert_roles()
    ...
    def test_ping(self):
        """effective of User.ping() - User.last_seen update"""
        u = User(password='cat')
        db.session.add(u)
        db.session.commit()
        time.sleep(2)
    ...
    def test_follows(self):
        ...
        db.session.add(u1)
        db.session.add(u2)
        db.session.commit()  # commit at once to generate User.id
        ...
        f = u1.followed.all()[-1]  # the last one
        ...
        self.assertTrue(Follow.query.count() == 2)  # 2 self-following
        # test delete-orphan option
        u2.follow(u1)
        db.session.add(u1)
        db.session.add(u2)
        db.session.commit()
